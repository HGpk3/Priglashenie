...
Перед отправкой сервер валидирует обязательные поля, проверяет допустимые варианты ответов, ограничивает частые повторные отправки и отсекает простые спам-боты через скрытое поле.
...
Для настоящей отправки RSVP обязательно заполнить `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` в `.env`. Если они пустые, форма покажет понятную ошибку и не потеряет ответ молча.
